...
#include "../../include/experiment/ExperimentNoDB.h"
...
Experiment::parse_params(int total_prams, char **pram_list) {
  bool argument = false;
  ...
  for (int temp = 0; temp < total_prams; temp++) {
    ...
    if (s.find("--") != std::string::npos) {
      ...
      result.insert(
          std::pair<std::string, std::vector<std::string>>(key, my_vec));
    } else if (argument) {
...
std::vector<int> Experiment::frequency_of_params(
...
ExperimentJSON::get_args_for_run(nlohmann::json j, int rank) {
  ...
  for (auto it = j.begin(); it != j.end(); ++it) {
    ...
    if (it->is_primitive()) {
      ...
      my_map.insert(
          std::pair<std::string, std::string>(it.key(), (*it)[temp_index]));
      ...
      //      val val " << (*it)[inner_index].back() << std::endl;
      //      my_map.insert(std::pair<std::string, std::string>(it.key(),
      //      (*it)[inner_index]));
...
ExperimentJSON::get_all_keys(nlohmann::json j) {
...
ExperimentJSON::ExperimentJSON(std::string file_name) {
  using namespace nlohmann;
  std::ifstream myfile;
  myfile.open(file_name);
  nlohmann::json j;
  ...
  args_for_run.insert(
      std::pair<std::string, std::string>("run", std::to_string(run)));
  //  std::cout << "PRINTING MAP PRINTING MAP PRINTING MAP\n";
  //  for (auto it = args_for_run.begin(); it != args_for_run.end(); ++it) {
  ...
  for (auto const &imap : all_keys) {
    ...
    if (!imap.second.empty() &&
        imap.second.find_first_not_of("-0123456789") == std::string::npos) {
      types.emplace_back("int");
    } else if (!imap.second.empty() &&
               imap.second.find_first_not_of("-.0123456789") ==
                   std::string::npos) {
...
Experiment::Experiment(int argc, char *argv[]) {
  ...
  if (this->args.count("run")) {
    ...
    std::cout << "Run number not provided; for example, pass --run 0 as "
  ...
  for (auto &arg : this->args) {
    this->args_for_run.insert(std::pair<std::string, std::string>(
        arg.first, arg.second[selected_combinations[temp_counter]]));
    std::cout << arg.first << " "
              << arg.second[selected_combinations[temp_counter]] << std::endl;
    temp_counter++;
  }
...
int Experiment::get_int_param(const std::string &param) {
  //    std::cout << "Param count " << param << this->args_for_run.count(param)
  //    << " " << std::endl;
